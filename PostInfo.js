...
useEffect(() => {
  setDate(route.params.date_exp)
  //date is a varaible
  //setDate(value you want date to now be)
  fetch(`${domain}/api/auth/posts/${route.params.id}`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json'
...
function changeReciever(user) {
  ...
  // }
    return (
        ...
        <View style ={{backgroundColor: "rgba(38,154,144,1)", marginLeft : 10, marginRight :10, marginTop : 10}}>
            <Text style = {styles.header}>
                Post Information
            </Text>
            <Text style = {styles.dateofexp}>Welcome {userObj.first_name} {userObj.last_name}</Text>
            <View>
            <Text style = {styles.dateofexp}> {route.params.header_text}</Text>
            <Text style = {styles.dateofexp}>Description: {route.params.body_text}</Text>
            <Text style = {styles.dateofexp} >Date of Expiration: {route.params.date_exp}</Text>
            <Text style = {styles.dateofexp}>Delivery Location: {[route.params.street, route.params.city, route.params.state, route.params.country, route.params.zipcode]}</Text>
            <Text style = {styles.dateofexp}>Producer: {route.params.produceremail}</Text>
...
const styles = StyleSheet.create({
    ...
    btn : {
        backgroundColor : 'rgba(227,109,47,1)',
        margin : '3%',
        padding: 15,
        marginVertical: 5,
        alignItems: 'center',
