...
function ListingPage({ navigation, route, props }){
  ...
  useEffect(() => {
    ...
    fetch(`${domain}/api/auth/posts/`, {
      ...
      .then(json => {
        //json = [{'date_finish': 907, 'producer': vijay@gmail.com}, {sdfafd}]
        //new Date() (not exactly sure if thats what you need to do)
        //get date_exp for each post, check if its less than curr date and time
        //if post is expired, dont let it be shown
  ...
  return (
    ...
    <View>
        ...
        {/* <Button title = "Add A Friend to Your Group List" onPress = {() => navigation.navigate("CreateMember")}/>
        <Text></Text>
        <Button title = "View Group List" onPress = {() => navigation.navigate("GroupList")}/>
        <Text></Text> */}
